...
$(document).ready(function(){
	...
	dropDown('.jieqinAddrContent a','.jieqinAddr');//接亲地址
...
function addOrder(indentNo){
	if(state==1){
		...
		var indentManagerPhone=$(".indentManagerPhone input").val();//酒店经理电话
		var indentRemarks=$(".indentRemarks").val();//备注
		var total=$(".total_calc_Total").text();//合计
		//酒店地址
		...
		//接亲地址
		var dropdown_x50=$(".dropdown_x50").text();//省
		var dropdown_x60=$(".dropdown_x60").text();//市
		var dropdown_x70=$(".dropdown_x70").text();//区
		var dropdown_x80=$(".dropdown_x80 input").val();//详细地址
		if(!indentTime){
			meg("提示","请选择仪式时间","body");
			return false;
			...
			meg("提示","请填写新郎姓名","body");
			return false;
		}else if(!indentBridePhone){
			meg("提示","请填写新娘电话","body");
			return false;
		}else if(!indentBridegroomPhone){
			meg("提示","请填写新郎电话","body");
			return false;
		...
		for(var x=0;x<$(".Reimbursement li").length;x++){
			if($(".Reimbursement li").eq(x).hasClass('Reimbursement_on')){
				indentRbb.push($(".Reimbursement li").eq(x).find("span").text());
			}
		}
		var data = {
			indentNo:indentNo,
			...
			indentManagerPhone:indentManagerPhone,
			indentDetailedHotelAddress:dropdown_x10+','+dropdown_x20+','+dropdown_x30+','+dropdown_x40,
			indentDetailedGreetAddress:dropdown_x50+','+dropdown_x60+','+dropdown_x70+','+dropdown_x80,
